...
function getPolynom(...coeff) {
  if (coeff.length !== 0) {
    return function polynom(x) {
      return coeff.reduce((result, current, index) => {
        const power = coeff.length - 1 - index;
        return result + current * (x ** power);
      }, 0);
    };
  }
  return null;
...
function logger(func, logFunc) {
  return function Log(...args) {
    const newArg = args.map((arg) => JSON.stringify(arg)).join(',');
    logFunc(`${func.name}(${newArg}) starts`);
    const result = func.apply(this, args);
    logFunc(`${func.name}(${newArg}) ends`);
    return result;
  };
}
